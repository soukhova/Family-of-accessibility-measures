---
title: "Accessibility, constraints and the family of spatial interaction models" #"A family of accessibility measures (bringing balance back?)"
format: pdf
...
In adopting the perspective that accessibility is an evolution of mobility conceptually, this paper aims to re-unify accessibility measures with its spatial interaction roots. It proposes to do so by mapping accessibility onto Wilson's family of spatial interactions models [@wilson1971]. In this, we aim not only to clarify accessibility's interpretation as a measure of performance, but also to encourage future developments that draw from its spatial interaction history. To this end, we begin by tracing the development of spatial interaction from @ravensteinLawsMigration1889 to its split into accessibility research from @hansen1959 to today. After this, we derive different types of accessibility measures, following the family of "spatial interaction models" (SIM) namely: unconstrained, total constrained, origin- and destination- constrained, composite destination-constrained, and doubly constrained. Then, we will provide empirical examples of the types of measures. Finally, we will discuss the use of each measure and broadly the implications of ignoring the role of constrains in accessibility analysis.
...
<!--SUMMARY: State how accessibility has the same Newtonian roots as Wilsons' family of spatial interaction models. Introduce SIM's formulation. Emphasis the interpretation that comes from $k$. Emphasis how while SIM has Newtonian roots, its proportionality constant(s) are actually balancing factors derived via entropy-maximisation, obtaining the interaction variable as a statistical average. -->
Today's gravity-based accessibility measures (@eq-accessibility-general) share the same early 20th century Newtonian-based origins as the model's of Reilly, Zipf, and Stewart, versions of which continue to be extensively used today in transportation demand modelling [@ortuzar_2011_modelling;@wilson1971]. However, in many ways these spatial interaction models (AKA gravity models) continued to evolve independently from accessibility (@eq-accessibility-general) as will be discussed next section. Our paper aims to demonstrate that Hansen-type accessibility can be interpreted as a SIM. This connection can enrich the accessibility literature by linking it to the extensive practical application of SIMs. This paper's focus is on how SIMs utilize "proportionality constants" and constraints to interpret results, and how Hansen-type accessibility can transit to _constrained_ accessibility. 
To begin, the SIM as defined in Wilson [-@wilson1971], takes the following form: 
$$
...
Where $T_{ij}$ represents a $n$ x $m$ matrix of flows between an $n$ number of origins $i$ and an $m$ number of destinations $j$, $W_i^{(1)}$ and $W_j^{(2)}$ is a vector of mass attributes representing origin $i$ and destination $j$ respectively, and $f(c_{ij})$ is a $n$ x $m$ matrix representing some function of travel cost $c_{ij}$ which reflects travel impedance. In other words, $T_{ij}$ explicitly measures _interaction_ as a unit of trips and the single proportionality constant $k$ ensures the total sum of $T_{ij}$ represents the total flows in the data. It should be noted that the early analogy with the gravitational law proved too simplistic, and improvements to the SIM (@eq-phys-gravity-model) are the use of total trip ends (number of originating trips from zone $i$ are $O_i$ and ending trips at zone $j$ are $D_j$) instead of total populations and separation distance calibrated by a power $n$ (such as $P_i$, $P_j$, and $n$ in @eq-zipf, @eq-reilly and @eq-stewart-force). Finally, and of most important note in this paper, $k$ is a scale parameter that makes the overall equation proportional to the rate characteristic of the modeled phenomenon.

Traditionally, the development of the SIM has put an emphasis on the interpretability of the resulting units [@kirbyNormalizingFactorsGravity1970; @wilsonSTATISTICALTHEORYSPATIAL1967;@wilson1971]. But instead of relying on the heuristic Newtonian gravity analogy (e.g., some interaction between a mass at $i$ and a mass at $j$ seperated by some distance), entropy maximisation has emerged and continues to be a popular model building approach to generate the SIM [@ortuzar_2011_modelling]. 

Namely, the entropy maximisation approach achieves stable results as a statistical average that represents the population. The approach works with individual interaction, assesses their probability of making a particular journey given given zonal and regional constraints, and the resulting interaction from @eq-phys-gravity-model is a statistical average [@wilson1971;@seniorGravityModellingEntropy1979]. For instance, the interaction variable $T_ij$ is often in the unit of trips, so additional knowledge can be the total trip flows originating at each zone $W_i^{(1)}$ and the total trip flows terminating at zone $W_j^{(2)}$ can be integrated into zonal constraints as follows.

The total flow constraint: @eq-constraint0-gravitymodel, where the total number of flows in the region equals $T$: <!--NOTE: in Wilson's entropy-max derivation, this is actually eq-constraint0_5-gravitymodel:-->
$$
\sum_i\sum_j T_{ij} = T
...
- The total constrained case: only @eq-constraint0-gravitymodel hold <!--NOTE: not sure if this is consistent with entropy-maximisation?-->
...
<!--NOTE: pretty sure K is not consistent with entropy-maximisation?-->
$$
...
## Background on accessibilities' distinct development away from spatial interaction models
<!--COME-BACK HERE-->
...
The institutionalization of research on spatial interaction modelling was fueled by advancements in computer technologies and the substantive and unprecedented Federal-Aid Highway Act of 1956, which authorized $24.8 billion in spending at a 90% federal share from between 1957 to 1969 to begin and complete the US Interstate Highway System [@weinerUrbanTransportationPlanning2016]. This ambitious project went on to shape transportation planning practice globally []. Within the same time period, accessibility measures were not applied practically at such a large scale. Instead, accessibility measures were taken up to describe land-use and transportation interactions by geography and social science-adjacent researchers and planners; remaining more in theory than in practice.  However, due to growing critiques about traditional transportation modelling practice focus on  _mobility_, calls for a shift to accessibility are intensifying [@handy2020] and accessibility planning is beginning to be present in practice [] <!-- citation on where they are applied in practice? US guidelines. Netherlands. -->

Despite the theoretical and observed reality that both mobility and accessibility influence an individual’s capacity to travel [@morrisAccessibilityIndicatorsTransport1979], SIM literature that focuses on Hansen's accessibility is relatively scarce in the literature. To capture this sentiment in the literature, @fig-Fig1 demonstrates the bibliometric coupling of papers that cites @hansen1959, @wilson1971, and both @hansen1959 and @wilson1971. @fig-Fig1 is based on a SCOPUS search of papers that cite @hansen1959's DOI (2,134 documents) and another SCOPUS search of papers that cite @wilson1971's DOI (368 documents) conducted on November 9, 2024 and a bibliometric coupling matrix created using the {bibliometrix} R package [@bibliometrix2017].
...
Many of the papers that cite both @hansen1959 and @wilson1971 (77 journal articles) are transportation modelling papers working on integrating accessibility within the SIM or interpreting HAM within the SIM. For instance, @battyMethodResiduesUrban1976 states that the relationship between the SIM and HAM may be worth exploring. There is also a consistent acknowledgement in the literature that HAM and SIM are related. Specifically, that HAM are within the balancing factors of the SIM [@harrisEquilibriumValuesDynamics1978; @leonardiOptimumFacilityLocation1978; @fotheringhamSPATIALSTRUCTUREDISTANCE1981; @fotheringhamSpatialCompetitionAgglomeration1985]. Recognizing the balancing factors as HAM is a "common sense" approach at integrating accessibility in the SIM [@morrisAccessibilityIndicatorsTransport1979]. 

There are other approaches to consider accessibility with SIM beyond HAM which can be founded on micro-economic consumer behaviour [@morrisAccessibilityIndicatorsTransport1979]. For instance, others explore entropy maximizing solutions (Wilson's framework and hence the balancing factors defined as Hansen's accessibility) when considering demand behavior described by random utility choice models [@leonardiRandomUtilityDemand1984].

Some work on location-allocation models also used both Hansen and Wilson. For instance, @leonardiOptimumFacilityLocation1978 and @beaumontLocationallocationProblemsPlane1981 define a location-allocation problem using the singly-constrained SIM, and then defines one using Hansen-type accessibility as the objective function subject. 

Then, @fotheringhamSpatialCompetitionAgglomeration1985 demonstrated that destination's accessibility should be explicitly specified within Wilson's framework, citing Hansen measure as convenient formulation. Without the consideration for accessibility explicitly, unexplained spatial pattern remains. This later came to be the 'competition destination' model, and is one of a few techniques to address spatial variation in SIMs. [@oshanSpatialStructureDebate2021]. Implicitly, we gather that at this point: it was understood balancing factors in the SIM could not be understood as capturing accessibility. We also gather that in this moment of growing complexity of spatial interaction modelling, accessibility branched off and sprouted in the medium of budding GIS technology and the emergence of space-time dimensions [@millerMeasuringSpaceTimeAccessibility1999; @occelli2000revisiting]. With the possibility for showcasing complex GIS methodologies for varied case studies, accessibility became mature enough to branch off on its own. 
...
Some papers also use SIMs to estimate trip or some other interaction flows alongside accessibility indicators [@clarke2002deriving; @grengs2004measuring; @turk2019socio] or integrate accessibility within the SIM, in line with @fotheringhamSpatialCompetitionAgglomeration1985's demonstration and most often in context with retailing [@beckers2022incorporating].
...
<!-- NOTE: you can validate SIM e.g., how well does the model predict a known Tij matrix? but what is a known Vij matrix, potential accessible opportunities at i for j? Harder to say. -->
...
## Defining accessibility constraints (a potential spatial interaction model?)
...
Solved for our example, $K$ is equal to the total accessibility $V$ (ALL THE OPPORTUNITIES, OR ALL THE DESTINATIONS), divided by the sum of the unconstrained accessibility:
$$
...
\noindent Where $A_{ij}$ and $B_{ij}$ are similar to the balancing factors of the SIM for the singly-constrained cases, though with an addition to the numerator to maintain the associated enforcing constraints:
$$
...
Returning to our simple example, the opportunity-constrained singly-constrained case would yield the following $B_{ij}$ values:
$$
...
Example of solving the population-constrained $V_i''^{(1)}$:
<!--COMEBACKHERE-->
