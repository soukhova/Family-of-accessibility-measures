---
title: "A family of accessibility measures and the concept of potential derived from spatial interaction principles"
format:
  pdf:
    header-includes:
        - \usepackage{float}
...
<!-- RP: I say we spell it out as in this eq^, what you think? AS: Maybe! -->
...
On the other side of the Atlantic, Alan G. Wilson was developing related, yet parallel work. In the groundbreaking [@wilson1971], Wilson defined a general spatial interaction model as follows. While accessibility was characterised as an associated concept of 'potential', the primary focus was on modelling spatial interaction:

On the other side of the Atlantic, Alan G. Wilson was developing related, yet parallel work. In his groundbreaking study [@wilson1971], accessibility is acknowledged as an associated concept, though the work's primary focuses on defining an introducing a general spatial interaction model as follows:
...
In the following subsections, we detail the implications of introducing the constraint $k$ into accessibility measures and how different _constrained_ accessibility measures can be defined based on the focus of measurement and the input population and opportunity data. As an overview, the way each constraint $k$ is defined delineates one of four distinct members of the constrained accessibility measure family: 

1. The unconstrained case. This case is equivalent to @hansen1959's formulation whereby a balancing factor is neglected, so units of zonal values are simply in 'opportunities-by-travel-impedance-value'; 
2. The totally-constrained case; maximum potential. Two variants are introduced. 
  - The first, the _totally-constrained accessible opportunities measure_, resembles @hansen1959's formulation but with an additional regional balancing factor $K^T$ term, defined as the ratio of the total number of opportunities in the region to the total sum of unconstrained accessibility values in the region. In this way, $K^T$ ensures that each zonal accessibility value is a proportion of the total opportunities in the region, requires no information about the population seeking opportunities, and is in units of 'opportunities' accessible. 
  - The second variant is the transpose of $i$ to $j$ of the first variant, effectively a constrained version of market potential and is referred to as _totally-constrained accessible population measure_. In this variant each zonal accessibility value is a proportion of the population in the region, requires no information about the opportunities that are sought, and each zonal value is in units of 'population' accessible;
3. The singly-constrained case; medium potential, potential is limited by either an opportunity-constraint or population-constraint. Similarly, two variants are introduced. 
  - The first is _attraction-constrained_ from the perspective of spatial interaction modelling. This variant is refereed to as the _singly-constrained accessible opportunities measure_. It can also be formulated to resembles @hansen1959's formulation but with an additional term for $k$, a set of balancing factors $B_j$ which ensure opportunities to each zone are allocated based on population and travel impedance regionally and zonally. $B_j$ ensures that a balanced proportion of the total opportunities at each destination is allocated based on zonal population and travel impedance, so that each zonal accessibility value reflects both a share of the total opportunities in the region _and_ is a sum of a _balanced_ proportions of opportunities allocated from each destinations. Similar to the totally-constrained case, each zonal accessibility value is expressed in units of 'opportunities' accessible. But unlike the totally-constrained case, the singly-constrained case explicitly incorporates population by allocating a balanced share of a destination's total opportunities to populations at reachable origins. This variant is mathematically equivalent to spatial availability [@soukhovIntroducingSpatialAvailability2023] and its per capita version is equivalent to the 2SFCA [@shenLocationCharacteristicsInnercity1998; @luo2003]. 
  -  The second variant is _production-constrained_ from the perspective of spatial interaction modeling. It can also be interpreted as a constrained form of market potential (i.e., the transpose of the singly-constrained variant), incorporating the assumptions of the single constraint. Specifically, each zonal accessibility value represents both a share of the total regional population and the sum of balanced proportions of population from all origins, allocated to each destination based on the opportunities sought and travel impedance. Each zonal value is expressed in units of 'population' accessible;
4. The doubly-constrained case. It is constrained simultaneously by population and opportunities, so each zonal value is expressed in units of 'population-opportunity capacity' that is accessible. This case requires the number of opportunities and population to match, e.g., the user must know the matching spatial interaction capacity of the population (demand) and opportunities (supply). By using this one-to-one matching data, this case's calculated values especially limits the 'potential' to spatially interact compared to the data used in the singly-constrained and totally-constrained cases. Instead, the doubly-constrained values reflect the number of predicted interactions between the opportunities and population. Effectively, this case is equivalent formulaically as Wilson's doubly-constrained spatial interaction model (i.e., _attraction-production constrained_) and can be understood as predicting a value of access , not accessibility.  

In the next section, we illustrate each case using a simple numerical example.
...
While one could attempt to adjust the units post-calculation (e.g., scaling, population normalization) or select impedance functions to facilitate comparison across scenarios (potentially at the expense of accurately reflecting travel behavior), such adjustments may introduce bias. Hence, the raw unconstrained accessibility values themselves are challenging to compare due to their units. To enable more meaningful comparison, the following sections will detail the introduction of constraints to ensure consistent units across scenarios and demonstrate results on the same numeric example.

## Totally-constrained accessibility

In the totally-constrained accessibility case, a total constraint proportionally adjusts unconstrained zonal accessibility values $V^0_i$ based on the regional sum of $V^0_i$ and the total population or opportunities in the region. Alternatively, we reformulate this case using a proportional allocation constant, which allocates opportunities (or population) proportionally based on the the travel impedance and the total population or opportunities in the region. In both formulations, all zonal values become a proportion of a known system total, be it the regional opportunities or regional population depending on the variant.

We define two variants for this case: (a) case where accessibility is constrained by the total number of opportunities (totally-constrained accessible opportunity) and which is interpreted as Hansen's accessibility with a constraint, and (b) a variant where $i$ and $j$ of the totally-constrained accessibility measure is transposed (totally-constrained accessible population) hence is constrained by the total number of population and interpreted as constrained 'market potential'.

### Totally-constrained accessible opportunities: Hansen's accessibility with a constraint

Unlike in @eq-access-01, the proportionality constant $k$ is retained. For the totally-constrained case, it is represented as $K^T$:

$$
V^T_{ij} = K^T \cdot W_j^{(2)} \cdot f(c_{ij})
$$ {#eq-total-constrained-access}

In this way, the totally-constrained accessibility measure now becomes Hansen's accessibility with a proportionality constant $K^T$:

$$
V^T_i = \sum_j V^T_{ij} = K^T \sum_j W^{(2)}_jf(c_{ij}) = K^T \cdot V^0_i
$$ {#eq-total-constrained-accessibility}

Imagine that the only system known is the total number of opportunities $D$ in the region. Accordingly, the constraint we impose in this case ensures the regional sum of totally-constrained accessibility is equal to the total number of opportunities as follows:
$$
\sum_i V^T_{i} = \sum_i\sum_j V^T_{ij} = D
$$ {#eq-total-constraint-accessibility}

<!-- rafa: do we need to keep equations 31 and 32. AS: I think yes. -->
...
Which is also congruent with Wilson's framework as it comparable to the total flow spatial interaction model (e.g., Equation 2.11 in @cliff_evaluating_1974). Hence, rearranging the equation to have opportunities and the proportional constant distinctly represented, our totally-constrained accessibility model is:

$$
V^T_i = K^T\sum_j W^{(2)}_jf(c_{ij}) = \sum_j W^{(2)}_j\frac{D\cdot f(c_{ij})}{\sum_i\sum_j W^{(2)}_jf(c_{ij})} 
$$

Further, we can see that, since $D$ and $W^{(2)}_j$ are both in units of opportunities, the proportional allocation factor for the totally-constrained opportunity case $\kappa^T$ is dimensionless:

$$
\kappa^T = \frac{D\cdot f(c_{ij})}{\sum_i\sum_j W^{(2)}_jf(c_{ij})}
$$

\noindent and therefore $V^T_i$ is now in the units of $W^{(2)}_j$, that is, the mass at the destination ($V^T_i = \kappa^T\sum_j W^{(2)}_j$). The role of $\kappa^T$ in this reformulation of accessibility is to transform between units and to adjust the number of opportunities accessible from $i$ so they represent a proportion of the total number of opportunities in the region. Constant $\kappa^T$ then assigns opportunities in proportion to the impedance between $i$ and $j$. For this reason, we refer to $\kappa^T$ as a proportional allocation factor. On the other hand, the proportionality constant $K^T$ balances the units of $V^0_i$, the Hansen-type accessibility values, and is an alternative expression of the totally-constrained accessibility measure.

Referring back to our simple numeric example, proportionality constant $K^T$ for the most decay travel behaviour scenario $f_1(c_{ij}) = 1/c_{ij}^3$ would then be: 
...
### Totally-constrained accessible population: totally-constrained market potential

Another variant of the totally-constrained accessibility measure is the _totally-constrained accessible population measure_, which represents the transpose of $i$ to $j$ of the totally-constrained accessible opportunities measure. This variant, expressed in @eq-total-constrained-market, represents an expression of the concept of market potential (i.e., potential users) as proposed in @harris_market_1954 and @vickermanAccessibilityAttractionPotential1974. This unconstrained form of market potential $M_j^0$ (@eq-unconstrained-market), effectively the $i$ to $j$ transpose of $V^0_{ij}$, has been used in recent research to express the potentially accessible population (i.e., users) as a result of regional transportation infrastructure investment projects [e.g., @gutierrezLocationEconomicPotential2001; @holl2007twenty; @condecco2018road]. Put another way, market potential can also be thought of as a form of _passive accessibility_, indicating the number of people that can reach each destination. However, like $V_{ij}^0$, issues of unit interpretability arise in $M_j^0$'s unconstrained form. To address this, the constrained variant, the totally-constrained accessible population measure $M^T_{j}$, is introduced. To formulate this variant, the total constraint $K^T $ is applied to the mass of the *population* at $i$ ($W_i^{(1)}$) instead of the opportunities at $j$.
...
The totally-constrained accessible population measure (market potential) then becomes:
...
Readers may note the difference in trends in accessible population (@tbl-simple-example-total-population-accessibility) and accessible physicians (i.e., the preceding subsection, @tbl-simple-example-total-opportunity-accessibility). In @tbl-simple-example-total-opportunity-accessibility, zone 1, 2, 3 represent destinations and the accessibility values reflect the number of accessible people from the vantage of physicians. Zone 1, in its role as a destination, is no longer intermediately-ranked relative to other zones; it now attracts the fewest number of people across all three travel behaviour scenarios. However, similar to the totally-constrained opportunity case, as travel decay reduces, the availability of population begins to converge (though Zone 1 continues as the lowest-ranked) for similar reasons. As decay reduces, the population's travel impedance to all zones become more similar, making the relative location of the zones less important and all people in the region more equally accessible. Overall: like the totally-constrained accessible opportunities case, this variant allows for the interpretation of both ordinal and interval comparisons of the raw values themselves. 

## Singly-constrained accessibility

Similar to the totally constrained accessibility measure, the singly-constrained case includes a constraint that proportionally adjusts the unconstrained zonal accessibility values $V^0_i$. Two variants are defined: (a) the _singly-constrained accessible opportunities_  case (an alternative formula to spatial availability in @soukhovIntroducingSpatialAvailability2023), and (b) the _singly-constrained accessible population_ case, its transpose, or singly-constrained market potential.

Unlike the total constraint, the single constraint incorporates additional information. In the opportunities-accessible variant, the balancing factor constrains the 'potential' for spatial interaction by ensuring that only a proportional amount of opportunities at each destination are allocated to 'demanding' origins. This allocation is informed by demand, or the relative amount of population, and associated travel impedance connecting the zones. In the population-accessible variant (the transpose), population at each origin is proportionally allocated to destinations based on the share of opportunities and travel impedance.

In both variants, the balancing factor allocates opportunities (or population) based on the relevant supply-demand weights of the origins and destinations, and the travel impedance between zones. The singly-constrained accessibility measure introduces population-based (or opportunity-based) competition at the zonal level, unlike the total constraint, which more simply allocates a fixed regional total of opportunities (or population depending on the variant).

In sum, all singly-constrained zonal accessibility values are both a proportion of the known regional opportunity (or population) total _and_  a sum of a _balanced_ proportion of opportunities allocated from each destinations (or population allocated from each origin). Each zonal value remains in units of opportunities accessible (or population accessible), but in contrast to the totally constrained case, the singly constrained measures explicitly incorporates both spatial weights (i.e., population and opportunities) into the allocation process.

### Singly-constrained accessible opportunities: spatial availability

To demonstrate this variant formulaically, we begin with the opportunity constraint (@eq-opportunity-constraint) and our known piece of information, i.e., the sum of accessible opportunities from a destination should equal the number of opportunities $D_j$ at that destination. As the number of opportunities at each $j$ are known, it is represented as $D_j$ instead of $W_j^(2)$ as in the totally-constrained case. This constraint should hold for all destinations in the region. This is comparable to the single attraction-constraint (@eq-constraint2-gravitymodel) from Wilson's framework.
...
This simple example was constructed so that the regional average equals `r (opp_acc$V_opp_i_1|>sum()) /(opp_acc$O_i|>as.numeric()|>sum())` physicians per 10,000 people. As distance decay decreases and becomes _relatively_ uniform (all zones can reach all zones), the effect of population drives the proportional allocation of opportunities. Consequently, per capita accessibility values begin to stabilise to the regional per capita average (e.g., in the lowest distance decay $f_3(c_{ij})$, per capita values are all around 24 physicans accessible per capita). 

This trend mirrors how the accessibility values in the totally-constrained opportunity case stabilizes to the $V_i^T$ regional average (e.g., the accessible opportunities allocated to each of the three zones approaches a third of 490 physicians, or 163.33, under the unfettered mobility scenario $f_3(c_{ij})$). These patterns make intuitive sense: constraints act as regional and/or zonal averaging mechanisms. In distance decay travel behaviour scenarios that are uniform (i.e., low for all like in $f_3(c_{ij})$), what remains is the relative effect of the other variables in the constraint variable. In the totally constrained case, this is the proportion of opportunities relative to the regional opportunities, and in the case of the single opportunity constraint, this is the population at a zone relative to the regional population.

### Singly-constrained accessible population: singly-constrained market potential
...
Following the same logic as in the preceding section on totally-constrained market potential, one arrives at the following expression: 
...
As the doubly-constrained accessibility measure $V_{i}^D$ takes the form of the production-attraction (doubly-constrained) spatial interaction model, as shown in @eq-doubly-constrained-gravitymodel, $V_{i}^D$ is as follows:
...
Unlike in the totally- and singly-constrained cases, representing $V_i^D$ per capita is not as meaningful, as the value *already* matches population to opportunities, e.g., the number of opportunities accessed by the population. Similarly, the market potential form $M^D_j$ would be the transposed way of summarizing $V_{ij}^D$. In other words, the inputs of 'opportunities accessed' and 'accessed population' can already be interpreted as inherently being sensitive to both opportunities and population. As opportunities match population and both marginals are constrained, $M^D_j$ would be a summary of $V^D_{ij}$ values at each $j$, and would represent the number of people accessible to opportunities. The formulation is as follows:
...
## A note on interpretation and possible applications/ summary table?

<!-- Some Table here. AS example:
(col 1) type of measure, (col 2) explanation in plain language of the constraint, (col 3) continuum of potential (high to low), (4) equation of the constraint, (5) equation of the indicator, (col 6) input data needed?, (col 8)  opp-constrained value and interpretation of value (col 9)  pop-constrained value and interpretation of value -->

<!-- rafa: in the last sentence, you clarify the difference between 'total opportunity constraint' Vs  'single opportunity constraint' . What is confusing here is how the single opportunity constraint compares to 'total population constraint' . Again, the idea of including a table would be very helpful. The table shoud include one indicator in each row. The columns could include (1) name of the indicator, (2) explanation in plain language of the constraint, (3) equation of the constraint, (4) equation of the indicator, (5) example of the interpretation of the indicator.-->

unconstrainted: doesn't match the anything.

totally-constrained (opportunity): opportunities at all destinations can be accessed by all zones.
singly-constrained (opportunity): opportunities at each destination can be accessed by all populations.

totally-constrained (population): population at all origins can be accessed by all zones.
singly-constrained (population): population at each origin can be accessed by all opportunities.

doubly-constrained: opportunities at each destination can be accessed by all populations, while at the same time, the population at each origin can be accessed by all opportunities. i.e., a proportional amount of opportunities are allocated from each dest to origins (based on population demanding and travel impedance) and a proportional amount of population is allocated from each origin to dests (based on the opportunity supply and travel impedance).


# References

<!-- Of note RE DOUBLE CONSTRAINT: the accessibility case methodogy should be based selected based on the input data and the theory. For instance, the doubly-constrained accessibility method requires pportunities and population to matched one-to-one both mathematically (i.e., the balancing factors will not converge if they do not match) and theoretically (i.e., each person should seek one opportunity). By forcing these assumptions, the resulting accessibility value that is calculated can be interpreted with less 'potential', than in the totally- and singly- constrained cases using the unmodified example. This is because, the unmodified example inherently carries more 'potential', i.e., we only know where people are (who can potentially interact) and we know where there are opportunities (who can be potentially interacted with), but not the capacities of which this interaction happens.  As mentioned earlier, in accessibility work, the capacities of which population and opportunities interact is not often known: in fact, accessibility research is often interested in capturing potential at a less granular level: the _potential_ for a population (whether the have interacted, or have not interacted) to interact with opportunities - the capacity of this interaction is not often known, and not the focus.  -->

<!-- Using the doubly-constrained measure with the unmodified example is both mathematically impossible (as there is no one-to-one match) but also theoretically uncompitable as the capacities at both ends are not known. Conversely, the totally- and singly- constrained cases can be used to calculate values using inputs from the modified example, but, the results will be more impresice. If capacities of population and opportunities are both known, and the objective of the exercise is to calculate the number of _capacity_ of the opportunity (or population) that is accessible, than by using the singly-constrained case, capacities at  either the opportunity or population would be neglected. In using the totally-constrained case, capacities at both would be neglected. In this way, if the goal was to calculated acccessibility to opportunity or population capacity, the answer would be only a more imprecise estimation than if the doubly-constrained method was used.  -->
...
<!-- ## totally-constrained interaction case -->

<!-- In the totally-constrained case, $k$ is replaced with $K$ (@eq-bf-K-gravitymodel satisfying only constraint @eq-constraint0-gravitymodel) so the model looks as follows: -->
...
<!-- - totally-constrained has high error -->
