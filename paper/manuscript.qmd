...
# Gravity-based accessibility's sibling: Wilson's spatial interaction model
...
Unconstrained (note that there is no proportionality constant!):
$$
...
Total flow constrained (needs a single/global proportionality constant):
$$
...
Using these constraints, the gravity model @eq-phys-gravity-model is typically distinguished in four cases, cases that are mathematically and conceptually consistent, so they have continued to be useful in classifying the gravity model in trip distribution modelling to this day [@ortuzar_2011_modelling]: 

- None of @eq-constraint0-gravitymodel, @eq-constraint1-gravitymodel, and @eq-constraint2-gravitymodel hold -- the unconstrained case;
- Only @eq-constraint0-gravitymodel holds -- total flow constrained
