...
<!-- SUMMARY: Transportation planning, historically, has been mobility-oriented. Accessibility, however, should be the primary focus, and institutionalized mobility-based transportation practice has treated accessibility as it's proxy. This has had problematic consequences. Conceptually, accessibility is a more complete and comprehensive evolution of mobility. As such, there has been recognition that the adoption of accessibility planning approaches are needed; however there are 1) too many definitions and 2) uncertainty in meaning of the outputs. This work looks to demonstrate 1) that accessibility and mobility oriented models (i.e., gravity-based accessibility and the spatial interaction model) share the same roots in spatial interaction modelling, and accessibility can be specified within spatial interaction modelling's framework. and 2) demonstrate numerically how constrained accessibility can be specified, and in doing so brings interpretable units to each accessibility value, units of 'number of potential opportunities for spatial interaction'. 3) provide a clarifying framework for accessibility in the 'total constraint' and 'single constraint', explicitly connecting how the 'total constraint' connects with the dominant 'gravity-based' accessibility measure of Hansen and how population-competition is considered, as in the popular 2SFCA, in the the single constraint.-->
...
In the following subsections, we detail what are the implications of bringing this constraint into accessibility measures, and how various accessibility measures can be defined depending on selected constraints: how $k$ is specified and depending on the combination of population and opportunity data is available and measurement focus. Together, these constraints delineate different members of the family of accessibility measures, which fall under four categories: 

- (1) the unconstrained case. Equivalent to @hansen1959's formulation, a balancing factor is absent so units of zonal values are simply in 'opportunities-by-travel-impedance-value'; 
- (2) the totally-constrained case, which like @hansen1959's formulation but with a regional balancing factor $K$ that is a ratio of the total number of opportunities in the region to the total sum of unconstrained accessibility values in the region. In this way, $K$ ensures that each zonal accessibility value is a proportion of the total opportunities in the region and is in units of 'opportunities';
  - the inverse of this case, known as 'market potential' is also demonstrated, whereby each zonal accessibility value is a proportion of the population in the region, and each zonal value is in units of 'population';
- (3) the singly-constrained case, which can be opportunity-constrained, whereby a set of balancing factors $k_i$ are introduced to  ensure zonal accessibility values are a proportion of the total opportunities in the region, proportionally allocates opportunities based on population demand and travel impedance, and is in units of 'opportunities'. This case is mathematically equivalent to spatial availability [@soukhovIntroducingSpatialAvailability2023] and its per capita variant equivalent to the 2SFCA [@shenLocationCharacteristicsInnercity1998; @luo2003];
  - the 'market potential' variant of this case is also demonstrated, whereby each zonal accessibility value is a proportion of the population in the region constrained by the opportunity supplied and travel impedance, and each zonal value is in units of 'population';
- (4) the doubly-constrained case, which is constrained simultaneously by population and opportunities, and is in units of 'opportunities' (or 'population' as, effectively, they should match). The doubly-constrained accessibility case is effectively equivalent to Wilson's doubly-constrained spatial interaction model; collapsing 'potential' to interact with opportunities into simply being the number of predicted opportunities / population that are spatially interacted.

<!--COME BACK HERE, double check the para above-->
In the next section, we use a simple numerical example to illustrate these cases in the order described.
...
$$
>>>>>>> ac7b0b2b2c239082d42ca54a294d229e7100fdee
\begin{array}{l}
...
In contrast to unconstrained accessibility, imposing a constraint -in this case a total opportunity constraint- allows for the comparison of differences and ratios between regions and across different travel behaviour scenarios as well. Each value is effectively in units of physicians, with the impedance units already accounted for by $\kappa^T$.

Consider the highest decay scenario ($f_1(c_{ij})$), zone 1 again captures an intermediate amount of physicians (`r tot_acc$V_tot_i_1[1]`). However, we can say that this is out of the `r tot_acc$V_tot_i_1 |> sum()` physicians in the region, which allows us also to deduce that zone 1 captures `r tot_acc$V_tot_i_1[1]/tot_acc$V_tot_i_1[2]` and `r tot_acc$V_tot_i_1[1]/tot_acc$V_tot_i_1[3]` times more than zone 2 and 3. Values for the lesser decay ($f_2(c_{ij})$) and lowest decay ($f_3(c_{ij})$) scenarios are calculated separately, with decay scenario values also summing to equal `r tot_acc$V_tot_i_2 |> sum()` physicians accessible in the region.
...
<!-- rafa: This is a very important takeaway. But this is true for any metric using the constant, right? 66666666 -->
...
$$
This measure is simply a form of passive accessibility, and indicates that each destination can be reached by an x number of people. The constraint that we impose in this case in practice means that the total market potential equals the total population $O$ in the region:
...
<!-- rafa: I really like this brief intro, but I think it needs to be more explicit in how the singly-constrained accessibility differs from the totally-constrained accessibility. If I understand it correctly, the totally-constrained accessibility simply uses a constant factor to make sure the final estimates respect the limit of the total number of people or opportunities, while the singly-constrained accessibility uses this constant BUT it also balances/reapportions the allocation of masses between origin-destinations based on travel costs. Is this correct ? -->
...
<!-- You mean that (a) the indicator uses an underlying assumption that the number of people and opportunities should match one-to-one; or (2) that it is *not* possible to calculate the indicator unless the number of people and opportunities match one-to-one ? -->
...
<!-- remind the reader, what is $V_{ij}^D$ ? -->

<!-- I believe the title pf table 10 is incorrect. Table 10 simply presents the balancing factors, right ? -->
...
<!-- rafa: if I understand it correctly, this paragraph is basically saying that the doubly-constrained accessibility measure is not an accessibility measure, and that it works best as a spatial interaction model that models the most likely distribution of trips between origins and destinations. Correct ? If so, this needs to be said right at the start of this sub-section -->
...
<!-- rafa: I like the idea of this paragraph above, but perhaps we need to articulate it more clearly. I understand you are saying that the analysis presented in this paper should help clarify that (1) gravity-based spatial interaction models and accessibility measures all derive from the same root, and (2) all of them can be derived from Wilson's original formulation; and (3) what differentiates such various spatial interaction models and accessibility metrics is simply the way in which proportionality constants (and balancing factors?) are explicitly or implicitly incorporated into their calculations -->


```{=html}
<!--
NOTE: competitive accessibility Literature has explored 'doubly-constrained' but -- it's not the same as this: they just use the Wilson balancing factors , etc. Ex. @hornerExploringMetropolitanAccessibility2004 and @allenMeasureCompetitiveAccess2020.
-->
```
...
In this paper, we have examined the historical and mathematical commonalities between spatial interaction models and place-based accessibility measures. As the field of transportation and accessibility research evolved largely influenced by Hansen's work, researchers in the field became oblivious along the way of a proportionality constant that was originally present in gravity-based models. In this study we have demonstrated that by reintroducing such constraints into accessibility analyses, we effectively derive a unifying family of accessibility measures that account for competition.

<!-- rafa: if you agree with the claim above, perhaps we need to mention at some point (section 'A family of accessibility measures') how the measures derived in this paper connect to the family of floating catchment area  metrics. I know the manuscript already says that 2SFCA is one particular case of a more general indicador, and this is great. But perhaps we need to articulate how BFCA fits in the story of this paper. And we might want to state that all other FCA metrics are simply 2SFCA with different decay functions in the end.... but this needs to be said. -->
...
<!-- rafa: this info should be part of the summary table, suggested by Chris  -->
