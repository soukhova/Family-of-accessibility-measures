...
More specifically, this work seeks to address the breath of place-based accessibility's definitions and lack of interpretability by demonstrating that mobility-oriented models--such as the commonly used gravity-based accessibility [e.g., @hansen1959] and the spatial interaction model [e.g., @wilson1971]-- are rooted in the same spatial interaction modeling foundation. In fact, accessibility can be specified using spatial interaction principles and a _family of measures_, akin to the family of spatial interaction models introduced in @wilson1971, can be defined using balancing factors that constrains values based on known information about the land-use.

In this way, this work aims to offer three contributions: (1) introduce and (2) formulaically define a family of _constrained_ accessibility measures, namely the total constrained accessibility measure, the singly constrained accessibility measure, and the doubly constrained measure. These measures are explicitly connected to popular measures such as the Hansen-type accessibility [@hansen1959], the popular competition approach of the 2-Step Floating Catchment Area (2SFCA) [@shenLocationCharacteristicsInnercity1998; @luo2003], and the concept of market potential, or what Reilly called 'potential trade territories' [@reilly1929methods] and further developed in other examples [@harris_market_1954; @vickermanAccessibilityAttractionPotential1974]. These Wilson-analogous balancing factors introduced are also defined in order of increasing restrictiveness to shed light on the role of _potential_ in accessibility and access. (3) This work also aims to demonstrate the intuitive value of restoring units to the resulting raw accessibility values. Each zonal and zonal flow value from a constrained accessibility measure is in interpretable units, namely the number of 'opportunities for spatial interaction' or 'population for spatial interaction'. This is in contrast to conventional accessibility measures that yield values in units of 'opportunities weighted by some representation of travel friction'. 
...
As an additional overview, we define four distinct members of the constrained accessibility measure family, all delineated based on their constant $k$; either balancing factors $K^T$, $B_j$, $A_i$ depending on their case and variant:
...
$$
\kappa_i^T = \frac{D\cdot f(c_{ij})}{\sum_i\sum_j W^{(2)}_jf(c_{ij})}
$$
...
$$
\hat \kappa_i^T = \frac{O \cdot f(c_{ij})}{\sum_i\sum_j W^{(2)}_if(c_{ij})}
$$
...
And as a summary: each member of the family of accessibility measure is named, explained in plain language, alongside their balancing factor, proportional allocation factor and mathematical equation and value interpretations in Table 8. <!-- @tbl:summary-family-of-accessibility-measures.-->
