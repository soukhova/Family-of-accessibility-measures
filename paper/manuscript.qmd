---
title: "Accessibility, a type of spatial interaction model" #"A family of accessibility measures (bringing balance back?)"
format: pdf
...
<!--SUMMARY: Accessibility has the same Newtonian roots as Wilsons' family of spatial interaction models. Introduce SIM's formulation. Emphasize the interpretation that comes from $k$. While SIM has Newtonian roots, its proportionality constant(s) are actually balancing factors derived via entropy-maximisation, obtaining the interaction variable as a statistical average. -->
...
The total flow constraint: @eq-constraint0-gravitymodel, where the total number of flows in the region equals $T$: <!--NOTE: in Wilson's entropy-max derivation, this is actually eq-constraint0_5-gravitymodel a TOTAL TRAVEL COST constraint:-->
$$
\sum_i\sum_j T_{ij} = T
...
- The total constrained case: neither @eq-constraint1-gravitymodel, or @eq-constraint2-gravitymodel hold, only @eq-constraint0-gravitymodel - the total number of trips, is used;
...
In each of these cases, the single proportionality constant $k$ is replaced by balancing factors $A_i$, $B_j$, and $K$ as follows to ensure the case-specific constraints are satisfied:
$$
...
For the doubly-constrained case, $A_i$ and $B_j$ must be solved together along with $K$. For the singly-constrained cases, origin-constrained case replaces $B_j$ with 1 for all $j$ and $A_i$ is @eq-bf-A-gravitymodel, and the the destination-constrained case replaces $A_i$ with 1 for all $i$ and $B_j$ is @eq-bf-B-gravitymodel. For both singly-constrained cases, $K$ also holds. For the total constrained case, $A_i$ and $B_j$ are set to 1 and $K$ is the defining constraint. 

These constraints (@eq-bf-A-gravitymodel, @eq-bf-A-gravitymodel, and @eq-bf-K-gravitymodel) and the SIM @eq-phys-gravity-model can be expressed not just for the variable of _interaction_ (e.g., the number of trips) but also as a variable of _potential interaction_. Before detailing SIM's formulation for accessibility, however, background on the development of accessibility and SIM independent from eachother is explained. 

## Background on accessibilities' distinct development from spatial interaction models
<!--
NOTE: This section is a tough one... please suggest ideas...
...
=======
>>>>>>> 04fc02473a287085b2cb85d7cf29de05b18e24bc
=======
>>>>>>> 04fc02473a287085b2cb85d7cf29de05b18e24bc
...
$$
S_{i} = 1*\sum_j O_j f(c_{ij}) \\
S_{ij} = k W_i^{(1)} W_j^{(2)} f(c_{ij}) = 1*1*O_j f(c_{ij}) 
$$
...
$$
V_{ij}' = K W_i^{(1)} W_j^{(2)} f(c_{ij})\\
V_{i}' = K\sum_j W_i^{(1)} W_j^{(2)} f(c_{ij})
$$
...
$$
K = \frac{V}{\sum_{i}\sum_{j} 1* O_j f(c_{ij})}\\
...
$$
B_{j} = \frac{1}{\sum_i 1* P_i f(c_{ij})}\\
B_{1} = \frac{1}{P_1 f(c_{1,1}) + P_2 f(c_{1,2}) + P_3 f(c_{1,3})}\\
.., j=2, j=3\\
...
$$
A_{i} = \frac{1}{\sum_j 1* O_j f(c_{ij})}\\
A_{i=1} = \frac{1}{O_1 f(c_{1,1}) + O_2 f(c_{1,2}) + O_3 f(c_{1,3})}\\
.., i=2, i=3\\
...
Literature has explored 'doubly-constrained' but -- not the same: ,etc. 
