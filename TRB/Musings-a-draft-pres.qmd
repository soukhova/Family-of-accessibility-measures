---
title: "Some musings Family of accessibility measures"
format: revealjs
---
```{r}
...
#annotater::annotate_active_file() #Cool!
```
...
\noindent Where $A_i$ (A_i may not make sense..?) and $B_j$ are:
$$
A_i = \frac{O_j}{\sum_j O_j f(c_{ij})}\\
B_j = \frac{P_i}{\sum_i P_i f(c_{ij})}\\
$$
At the zonal value:
$$
...
\noindent Where $A_i$ and $B_j$ are (this may not make sense..)
$$
...
```{r}
V_orig <- (A_i) * (V_unc)  # V_unc = (1/C)* t(matrix(rep(W_j, 3), 3, 3)) 
...
And as we can observe, the $P_i$ values are equal to the known populations, while $O_j$ is not. (well.. this doesn't really happen, this A_i may not make sense)
