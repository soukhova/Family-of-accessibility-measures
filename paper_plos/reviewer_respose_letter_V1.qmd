...
Thank you for the kind words and for this helpful feedback. We've signfigicantly revised the manuscript, focusing on explaining things concisely and ensuring each paragraph is move forward the key points of each section. In summary:

- We've reduced the length of the manuscript from 34 to 28 pages (without references).
- We've removed tangential details and minimised the equations where possible, particularly in the “Early Research” section.
- We've streamlined the manuscript's text--to make each sections purpose more clear.
...
Thannk you for your time and effort in reviewing our manuscript. We appreciate the constructive feedback and comments. 

Thank you for this point. We agree, the inclusion of an empirical example would demonstrate the real-world applicability of the measures. However, due to the existing length of the document, the addition of an empirical example (even if we replaced the toy example) would still add additional length: we would need to add a condensed methods and data section, and dedicate more room for tests. 

The length of the current document is an issue, so the addition of additional pages isn't feasible--especially given the concern over paper length that was raised by Reviewer 1 and how we’ve already reduced it by six pages in response to these concerns. However, we've put great thought into how the manuscript could be restructured to included an additional empirical example, but it just isn't possible given the manuscript's current format. 

As such, we’ve clarified that the contribution of this work is primarily theoretical and methodological. We’ve also tempered earlier claims about the measure’s real-world applicability, instead allowing readers to consider potential uses and the potential value of reuniting accessibility measures with interpretable units. We plan to explore an empirical application in a future publication which make this contribution more practically clear--where we may have enough space to develop and explore an empirical example without shortchanging the historical context. 
...
Also, the double constraint ends up fixomg the aggregated zonal value to match the 'mass' at the zone itself (i.e., reproducing what we already imposed onto the model). In this way, the $ij$ flows are the only useful thing for analysis. This focus has not tradtionally occupied the research context of accessibility-based work.
