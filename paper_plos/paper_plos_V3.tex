...
\citep{stewartDemographicGravitationEvidence1948} who theorised and
...
as part of entropy-maximisation) produce interpretable, unit-consistent
...
\includegraphics[width=0.7\textwidth,height=\textheight]{paper_plos_V3_files/figure-pdf/fig-docs-per-year-1.pdf}
...
impedance functions \emph{physicians-minute\(^{-0.1}\)} and
\emph{physicians-minute\(^{-3}\)}), making the direct comparison
...
and 3 are 4.002, 9.996, and 6.002. The usefulness of the doubly
constrained measure lies in the interpretation as \(V_{ij}^D\) values;
the number of opportunities from zone \(j\) allocated to populations in
zone \(i\) shaped by both mass and travel impedance.
...
opportunities that matches our known population at \(i\). Following the
...
capacity, we would be modeling realised access flows based on known
...
focused on questions of `potential' not on predicting flows of realised
access.
